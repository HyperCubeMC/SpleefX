...
public class NumberDecreaseButton extends Button {
    ...
    public NumberDecreaseButton register(int minimum, NumberIncreaseButton increaseButton, ItemStack decreaseItem, BiConsumer<InventoryClickEvent, Integer> valueChange) {
        ...
        increaseButton.getItem().setAmount(value.getValue() < 1 ? 1 : value.getValue() > 125 ? 1 : value.getValue());
        decreaseItem.setAmount(value.getValue() < 1 ? 1 : value.getValue() > 125 ? 1 : value.getValue());
        addAction(event -> {
            ...
            event.getCurrentItem().setAmount(value.getValue() < 1 ? 1 : value.getValue() > 125 ? 1 : value.getValue() > 125 ? 1 : value.getValue());
            event.getInventory().getItem(increaseButton.getSlot()).setAmount(value.getValue() < 1 ? 1 : value.getValue() > 125 ? 1 : value.getValue());
        });
