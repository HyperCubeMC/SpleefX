 ...
 *
 * ******************************************************************************/
package io.github.spleefx.util.message;
...
public final class CommentedConfiguration extends YamlConfiguration {
    ...
    public static String sanitize(String path, String[] comments) {
        ...
        for (int i = 0; i < comments.length; i++) {
            String cmt = comments[i];
            String c;
            if (cmt.equalsIgnoreCase("<blank>"))
                c = "";
            else
                c = space.toString() + "# " + comments[i];
    ...
    private static String getSectionPath(CommentedConfiguration commentedConfig, String line, String currentSection) {
        //Removing all spaces and getting the value of the section.
        String newSection = line.trim().split(": ")[0];

        //Parsing some formats to make sure having a plain value.
