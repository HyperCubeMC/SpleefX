...
@PluginSubcommand(
        name = "shop",
        permission = "spleefx.splegg.shop",
        permissionAccess = PermissionDefault.TRUE,
        description = "Display the splegg shop GUI",
        requirePlayer = true
)
public class SpleggShopCommand implements CommandCallback {
    ...
    public void onProcess(CommandContext context) {
        context.player().openInventory(new SpleggShop.SpleggMenu(EXTENSION.getSpleggShop(), context.player()).createInventory());
    }
}
